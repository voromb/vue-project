...
function decode(text) {
  ...
  try {
    ...
    warn$12(`Error decoding "${text}". Using original value`);
  }
  return "" + text;
}
...
function isSameRouteLocationParamsValue(a, b) {
  return isArray2(a) ? isEquivalentArray(a, b) : isArray2(b) ? isEquivalentArray(b, a) : a?.valueOf() === b?.valueOf();
}
...
function resolveRelativePath(to, from) {
  ...
  if (!from.startsWith("/")) {
    warn$12(`Cannot resolve a relative location without an absolute path. Trying to resolve "${to}" from "${from}". It should look like "/${from}".`);
    return to;
  }
  if (!to) return from;
    ...
    if (typeof position.el === "string") {
      if (!isIdSelector || !document.getElementById(position.el.slice(1))) try {
        ...
        if (isIdSelector && foundEl) {
          warn$12(`The selector "${position.el}" should be passed as "el: document.querySelector('${position.el}')" because it starts with "#".`);
          return;
        }
      } catch (err) {
        warn$12(`The selector "${position.el}" is invalid. If you are using an id selector, make sure to escape it. You can find more information about escaping characters in selectors at https://mathiasbynens.be/notes/css-escapes or use CSS.escape (https://developer.mozilla.org/en-US/docs/Web/API/CSS/escape).`);
        return;
      }
    }
    const el = typeof positionEl === "string" ? isIdSelector ? document.getElementById(positionEl.slice(1)) : document.querySelector(positionEl) : positionEl;
    if (!el) {
      warn$12(`Couldn't find element using selector "${position.el}" returned by scrollBehavior.`);
      return;
    }
...
function parseQuery(search) {
  ...
  for (let i = 0; i < searchParams.length; ++i) {
    ...
    if (key in query) {
      ...
      if (!isArray2(currentValue)) currentValue = query[key] = [currentValue];
...
function stringifyQuery(query) {
  ...
  for (let key in query) {
    ...
    (isArray2(value) ? value.map((v) => v && encodeQueryValue(v)) : [value && encodeQueryValue(value)]).forEach((value$1) => {
      if (value$1 !== void 0) {
...
function normalizeQuery(query) {
  ...
  for (const key in query) {
    const value = query[key];
    if (value !== void 0) normalizedQuery[key] = isArray2(value) ? value.map((v) => v == null ? null : "" + v) : value == null ? value : "" + value;
  }
...
function onBeforeRouteLeave(leaveGuard) {
  if (!getCurrentInstance()) {
    warn$12("getCurrentInstance() returned null. onBeforeRouteLeave() must be called at the top of a setup function");
    return;
  }
  const activeRecord = inject(matchedRouteKey, {}).value;
  if (!activeRecord) {
    warn$12("No active route record was found when calling `onBeforeRouteLeave()`. Make sure you call this function inside a component child of <router-view>. Maybe you called it inside of App.vue?");
    return;
  }
...
function onBeforeRouteUpdate(updateGuard) {
  if (!getCurrentInstance()) {
    warn$12("getCurrentInstance() returned null. onBeforeRouteUpdate() must be called at the top of a setup function");
    return;
  }
  const activeRecord = inject(matchedRouteKey, {}).value;
  if (!activeRecord) {
    warn$12("No active route record was found when calling `onBeforeRouteUpdate()`. Make sure you call this function inside a component child of <router-view>. Maybe you called it inside of App.vue?");
    return;
  }
...
function canOnlyBeCalledOnce(next, to, from) {
  let called = 0;
  return function() {
    if (called++ === 1) warn$12(`The "next" callback was called more than once in one navigation guard when going from "${from.fullPath}" to "${to.fullPath}". It should be called exactly one time in each navigation guard. This will fail in production.`);
    next._called = true;
...
function extractComponentsGuards(matched, guardType, to, from, runWithContext = (fn) => fn()) {
  ...
  for (const record of matched) {
    if (!record.components && record.children && !record.children.length) warn$12(`Record with path "${record.path}" is either missing a "component(s)" or "children" property.`);
    for (const name in record.components) {
      let rawComponent = record.components[name];
      if (true) {
        if (!rawComponent || typeof rawComponent !== "object" && typeof rawComponent !== "function") {
          warn$12(`Component "${name}" in record with path "${record.path}" is not a valid component. Received "${String(rawComponent)}".`);
          throw new Error("Invalid route component");
        } else if ("then" in rawComponent) {
          warn$12(`Component "${name}" in record with path "${record.path}" is a Promise instead of a function that returns a Promise. Did you write "import('./MyPage.vue')" instead of "() => import('./MyPage.vue')" ? This will break in production if not fixed.`);
          const promise = rawComponent;
          rawComponent = () => promise;
        } else if (rawComponent.__asyncLoader && !rawComponent.__warnedDefineAsync) {
          rawComponent.__warnedDefineAsync = true;
          warn$12(`Component "${name}" in record with path "${record.path}" is defined using "defineAsyncComponent()". Write "() => import('./MyPage.vue')" instead of "defineAsyncComponent(() => import('./MyPage.vue'))".`);
        }
      }
      ...
      if (isRouteComponent(rawComponent)) {
        ...
        if (!("catch" in componentPromise)) {
          warn$12(`Component "${name}" in record with path "${record.path}" is a function that does not return a Promise. If you were passing a functional component, make sure to add a "displayName" to the component. This will break in production if not fixed.`);
...
function addDevtools(app, router, matcher) {
  ...
  setupDevtoolsPlugin({
    ...
    if (typeof api.now !== "function") warn$12("[Vue Router]: You seem to be using an outdated version of Vue Devtools. Are you still using the Beta release instead of the stable one? You can find the links at https://devtools.vuejs.org/guide/installation.html.");
...
function useHistoryStateNavigation(base) {
  ...
  function changeLocation(to, state, replace$1) {
    ...
    try {
      ...
      if (true) warn$12("Error with push/replace State", err);
  ...
  function push(to, data) {
    ...
    if (!history$1.state) warn$12("history.state seems to have been manually replaced without preserving the necessary values. Make sure to preserve existing history state if you are manually calling history.replaceState:\n\nhistory.replaceState(history.state, '', url)\n\nYou can find more information at https://router.vuejs.org/guide/migration/#Usage-of-history-state");
...
function createWebHashHistory(base) {
  ...
  if (!base.endsWith("#/") && !base.endsWith("#")) warn$12(`A hash base must end with a "#":
  ...
  function stringify(params) {
    ...
    for (const segment of segments) {
      ...
      for (const token of segment) if (token.type === TokenType.Static) path += token.value;
        ...
        if (isArray2(param) && !repeatable) throw new Error(`Provided param "${value}" is an array but it is not repeatable (* or + modifiers)`);
...
function createRouteRecordMatcher(record, parent, options) {
  ...
  if (true) {
    ...
    for (const key of parser.keys) {
      if (existingKeys.has(key.name)) warn$12(`Found duplicated params with name "${key.name}" for path "${record.path}". Only the last one will be available on "$route.params".`);
...
function createRouterMatcher(routes, globalOptions) {
  ...
  globalOptions = mergeOptions2(PATH_PARSER_OPTIONS_DEFAULTS, globalOptions);
  ...
  function resolve(location$1, currentLocation) {
    ...
    if ("name" in location$1 && location$1.name) {
      ...
      if (true) {
        ...
        if (invalidParams.length) warn$12(`Discarded invalid param(s) "${invalidParams.join('", "')}" when navigating. See https://github.com/vuejs/router/blob/main/packages/router/CHANGELOG.md#414-2022-08-22 for more details.`);
      }
      ...
      if (!path.startsWith("/")) warn$12(`The Matcher cannot resolve relative paths but received "${path}". Unless you directly called \`matcher.resolve("${path}")\`, this is probably a bug in vue-router. Please open an issue at https://github.com/vuejs/router/issues/new/choose.`);
...
function checkSameParams(a, b) {
  for (const key of a.keys) if (!key.optional && !b.keys.find(isSameParam.bind(null, key))) return warn$12(`Alias "${b.record.path}" and the original record: "${a.record.path}" must have the exact same param named "${key.name}"`);
  for (const key of b.keys) if (!key.optional && !a.keys.find(isSameParam.bind(null, key))) return warn$12(`Alias "${b.record.path}" and the original record: "${a.record.path}" must have the exact same param named "${key.name}"`);
}
function checkChildMissingNameWithEmptyPath(mainNormalizedRecord, parent) {
  if (parent && parent.record.name && !mainNormalizedRecord.name && !mainNormalizedRecord.path) warn$12(`The route named "${String(parent.record.name)}" has a child without a name and an empty path. Using that name won't render the empty path child so you probably want to move the name to the child instead. If this is intentional, add a name to the child route to remove the warning.`);
}
function checkSameNameAsAncestor(record, parent) {
  for (let ancestor = parent; ancestor; ancestor = ancestor.parent) if (ancestor.record.name === record.name) throw new Error(`A route named "${String(record.name)}" has been added as a ${parent === ancestor ? "child" : "descendant"} of a route with the same name. Route names must be unique and a nested route cannot use the same name as an ancestor.`);
}
function checkMissingParamsInAbsolutePath(record, parent) {
  for (const key of parent.keys) if (!record.keys.find(isSameParam.bind(null, key))) return warn$12(`Absolute path "${record.record.path}" must have the exact same param named "${key.name}" as its parent "${parent.record.path}".`);
}
function findInsertionIndex2(matcher, matchers) {
  ...
  if (insertionAncestor) {
    ...
    if (upper < 0) warn$12(`Finding ancestor route "${insertionAncestor.record.path}" failed for "${matcher.record.path}"`);
  }
  return upper;
}
...
function useLink(props) {
  ...
  const route = computed2(() => {
    ...
    if (!hasPrevious || to !== previousTo) {
      if (!isRouteLocation(to)) if (hasPrevious) warn$12(`Invalid value for prop "to" in useLink()
- to:`, to, `
- previous to:`, previousTo, `
- props:`, props);
      else warn$12(`Invalid value for prop "to" in useLink()
- to:`, to, `
- props:`, props);
      previousTo = to;
      hasPrevious = true;
...
function includesParams(outer, inner) {
  for (const key in inner) {
    ...
    } else if (!isArray2(outerValue) || outerValue.length !== innerValue.length || innerValue.some((value, i) => value.valueOf() !== outerValue[i].valueOf())) return false;
  }
  return true;
}
  ...
  setup(props, { attrs, slots }) {
    ...
    return () => {
      ...
      if (isBrowser && component.ref) {
        ...
        (isArray2(component.ref) ? component.ref.map((r) => r.i) : [component.ref.i]).forEach((instance) => {
...
function warnDeprecatedUsage() {
  ...
  if (parentName && (parentName === "KeepAlive" || parentName.includes("Transition")) && typeof parentSubTreeType === "object" && parentSubTreeType.name === "RouterView") {
    ...
    warn$12(`<router-view> can no longer be used directly inside <transition> or <keep-alive>.
...
function createRouter(options) {
  ...
  function addRoute(parentOrRoute, route) {
    ...
    if (isRouteName(parentOrRoute)) {
      ...
      if (!parent) warn$12(`Parent route "${String(parentOrRoute)}" not found when adding child route`, route);
      record = route;
  ...
  function removeRoute(name) {
    ...
    else if (true) warn$12(`Cannot remove non-existent route "${String(name)}"`);
  }
  function getRoutes() {
  ...
  function resolve(rawLocation, currentLocation) {
    ...
    if (typeof rawLocation === "string") {
      ...
      if (true) {
        if (href$1.startsWith("//")) warn$12(`Location "${rawLocation}" resolved to "${href$1}". A resolved location cannot start with multiple slashes.`);
        else if (!matchedRoute$1.matched.length) warn$12(`No match found for location with path "${rawLocation}"`);
      }
    ...
    if (!isRouteLocation(rawLocation)) {
      warn$12(`router.resolve() was passed an invalid location. This will fail in production.
- Location:`, rawLocation);
      return resolve({});
    }
    let matcherLocation;
    if (rawLocation.path != null) {
      if ("params" in rawLocation && !("name" in rawLocation) && Object.keys(rawLocation.params).length) warn$12(`Path "${rawLocation.path}" was passed with params but they will be ignored. Use a named route alongside params instead.`);
    ...
    if (hash && !hash.startsWith("#")) warn$12(`A \`hash\` should always start with the character "#". Replace "${hash}" with "#${hash}".`);
    ...
    if (true) {
      if (href.startsWith("//")) warn$12(`Location "${rawLocation}" resolved to "${href}". A resolved location cannot start with multiple slashes.`);
      else if (!matchedRoute.matched.length) warn$12(`No match found for location with path "${rawLocation.path != null ? rawLocation.path : rawLocation}"`);
    }
    return assign({
      fullPath,
      hash,
  ...
  function handleRedirectRecord(to, from) {
    ...
    if (lastMatched && lastMatched.redirect) {
      ...
      if (newTargetLocation.path == null && !("name" in newTargetLocation)) {
        warn$12(`Invalid redirect found:
      ...
      if (failure$1) {
        if (isNavigationFailure(failure$1, ErrorTypes.NAVIGATION_GUARD_REDIRECT)) {
          if (isSameRouteLocation(stringifyQuery$1, resolve(failure$1.to), toLocation) && redirectedFrom && (redirectedFrom._count = redirectedFrom._count ? redirectedFrom._count + 1 : 1) > 30) {
            warn$12(`Detected a possibly infinite redirection in a navigation guard when going from "${from.fullPath}" to "${toLocation.fullPath}". Aborting to avoid a Stack Overflow.
 Are you always returning a new location within a navigation guard? That would lead to this error. Only return when redirecting or aborting, that should fix this. This might break in production if not fixed.`);
  ...
  function navigate(to, from) {
    ...
    return runGuardQueue(guards).then(() => {
      ...
      guards = [];
      for (const record of enteringRecords) if (record.beforeEnter) if (isArray2(record.beforeEnter)) for (const beforeEnter of record.beforeEnter) guards.push(guardToPromiseFn(beforeEnter, to, from));
  ...
  function triggerError(error, to, from) {
    ...
    if (list.length) list.forEach((handler) => handler(error, to, from));
      ...
      if (true) warn$12("uncaught error during route navigation:");
      console.error(error);
    }
    ...
    install(app) {
      ...
      if (isBrowser && !started && currentRoute.value === START_LOCATION_NORMALIZED) {
        ...
        push(routerHistory.location).catch((err) => {
          if (true) warn$12("Unexpected error when starting the router:", err);
        });
      }
      const reactiveRoute = {};
